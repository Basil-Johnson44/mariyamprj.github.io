-- Fix ESP8266 status function and authentication issues
-- Schema Analysis: Existing auth system with user_profiles, iot_data table exists
-- Integration Type: Addition (new functions) + Fix (auth credentials)
-- Dependencies: iot_data table, auth.users
...
-- 2. Create weight threshold check function (if not exists)
...
-- 3. Fix authentication - Remove old demo credentials and add correct ones
...
-- 4. Create correct demo credentials
DO $$
DECLARE
...
BEGIN
    ...
    INSERT INTO auth.users (
        ...
        (captain_uuid, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated',
         ...
         '{"full_name": "Captain Seaguard", "vessel_number": "SG-001", "vessel_name": "S.E.A.G.U.A.R.D", "license_number": "ML-2024-SG001"}'::jsonb, 
        ...
        (officer_uuid, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated',
         ...
         '{"full_name": "First Officer", "vessel_number": "SG-001", "vessel_name": "S.E.A.G.U.A.R.D", "license_number": "ML-2024-OF001"}'::jsonb, 
         '{"provider": "email", "providers": ["email"]}'::jsonb,
         false, false, '', null, '', null, '', '', null, '', 0, '', null, null, '', '', null);

    -- Create user profiles (these will be created by trigger if it exists, or manually here)
    INSERT INTO public.user_profiles (id, email, full_name, role, created_at) VALUES
        (captain_uuid, 'CAPTAINSEAGUARD@gmail.com', 'Captain Seaguard', 'Admin', now()),
        (officer_uuid, 'officer@seaguard.com', 'First Officer', 'Officer', now())
...
-- 6. Create RLS policies for new functions (they're already SECURITY DEFINER, but ensure access)
-- Functions are accessible via RPC calls, no additional policies needed

-- 7. Add some fresh IoT data to test with
...
COMMENT ON FUNCTION public.get_esp8266_status() IS 'Returns ESP8266 connection status based on data freshness and quality';
COMMENT ON FUNCTION public.check_weight_threshold() IS 'Checks current weight against safety thresholds and returns motor timer info';